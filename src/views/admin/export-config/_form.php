...
/* @var $model nullref\documents\models\config\Export */
...
<div class="document-config-form">
    ...
    <?php if ($model->isNewRecord): ?>
        <?= $form->field($model, 'options[exporter]')->dropDownList(Helper::getExporterMap()) ?>
    <?php else: ?>

        <div class="row">
            <div class="col-md-4">
                <?= Html::label(Yii::t('documents', 'Options')) ?>
                <?= $form->field($model, 'options[exporter]')->hiddenInput()->label(false) ?>
                <?php foreach (Form::getWorkerOptions($model->getExporter()->getOptions()) as $key => $config): ?>
